...
def _test_tag_search_export(api: PRTAPI, fixtures: dict):
    ...
    # Validate JSON structure
    _validate_export_json_structure(
        export_data_loaded, "tags", "friend", len(export_data)
    )
...
def _test_note_search_export(api: PRTAPI, fixtures: dict):
    ...
    # Search for notes
    notes = api.search_notes(
        "meeting"
    )  # Should find notes with "meeting" in title/content
    ...
    if len(notes) > 0:  # Only test if we have notes
        ...
        # Validate JSON structure
        _validate_export_json_structure(
            export_data_loaded, "notes", "meeting", len(export_data)
        )
...
def _validate_export_json_structure(
    ...
    assert (
        export_info["search_type"] == search_type
    ), f"Search type should be '{search_type}'"
    ...
    assert (
        search_request["type"] == search_type
    ), f"Search request type should be '{search_type}'"
    assert search_request["term"] == query, f"Search request term should be '{query}'"
    assert (
        "executed_at" in search_request
    ), "Search request should have executed_at timestamp"
...
def _validate_contact_json_fields(contact: dict):
    ...
    assert (
        "has_profile_image" in contact
    ), "Contact should have 'has_profile_image' field"
    assert isinstance(
    ...
    if contact["has_profile_image"]:
        ...
        assert contact["exported_image_path"].endswith(
            ".jpg"
        ), "Image path should end with .jpg"

    # Ensure no binary data leaked into JSON
    assert (
        "profile_image" not in contact
    ), "JSON should not contain binary 'profile_image' data"
...
def _validate_exported_image(export_dir: Path, contact: dict):
    ...
    assert (
        100 < file_size < 1024 * 1024
    ), f"Image file size should be reasonable: {file_size} bytes"
