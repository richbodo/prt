...
from prt_src.config import data_dir, get_db_credentials, load_config, save_config
...
def setup_database(force: bool = False, quiet: bool = False) -> Dict[str, Any]:
    ...
    try:
        ...
        if not quiet:
            console.print(
                "Existing config is corrupt. Creating new config...", style="yellow"
            )
        config = {}
...
def initialize_database(config: Dict[str, Any], quiet: bool = False) -> bool:
    ...
    if db_file.exists():
        try:
            ...
            if not quiet:
                console.print(
                    f"Failed to connect to existing database: {e}", style="yellow"
                )
...
def setup(
    force: bool = typer.Option(
        False, "--force", help="Force regeneration of credentials"
    ),
    encrypted: bool = typer.Option(
        False, "--encrypted", help="Set up encrypted database"
    ),
...
def encrypt(
    db_path: Optional[Path] = typer.Option(
        None, "--db-path", "-p", help="Path to database file"
    ),
    key: Optional[str] = typer.Option(
        None, "--key", "-k", help="Encryption key (generates new one if not provided)"
    ),
    no_backup: bool = typer.Option(False, "--no-backup", help="Skip backup creation"),
    no_verify: bool = typer.Option(
        False, "--no-verify", help="Skip verification after encryption"
    ),
    force: bool = typer.Option(
        False, "--force", help="Force encryption even if already encrypted"
    ),
):
