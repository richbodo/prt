...
from prt_src.config import get_db_credentials
from prt_src.config import load_config
from prt_src.config import save_config
...
def encrypt(
    ...
    success = encrypt_database(
        db_path=db_path, encryption_key=key, backup=not no_backup, verify=not no_verify, force=force
    )
