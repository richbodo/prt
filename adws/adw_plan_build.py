...
from data_types import IssueClassSlashCommand
from dotenv import load_dotenv
from github import extract_repo_path
...
def check_env_vars(logger: logging.Logger | None = None) -> None:
...
def parse_args(logger: logging.Logger | None = None) -> tuple[str, str | None]:
...
def format_issue_message(
    adw_id: str, agent_name: str, message: str, session_id: str | None = None
) -> str:
...
def classify_issue(
...
) -> tuple[IssueClassSlashCommand | None, str | None]:
    ...
    logger.debug(f"issue_response: {issue_response.model_dump_json(indent=2, by_alias=True)}")
...
def check_error(
    error_or_response: str | None | AgentPromptResponse,
    issue_number: str,
    adw_id: str,
    agent_name: str,
    error_prefix: str,
...
def main():
    ...
    issue_plan_response: AgentPromptResponse = build_plan(issue, issue_command, adw_id, logger)
    ...
    implement_response: AgentPromptResponse = implement_plan(plan_file_path, adw_id, logger)
