...
from github import fetch_issue_comments
from github import fetch_open_issues
...
issue_last_comment: dict[int, int | None] = {}
...
def check_and_process_issues():
    ...
    try:
        ...
        if new_qualifying_issues:
            print(
                f"INFO: Found {len(new_qualifying_issues)} new qualifying issues: {new_qualifying_issues}"
            )
            ...
            for issue_number in new_qualifying_issues:
                ...
                if trigger_adw_workflow(issue_number):
                    ...
                    print(
                        f"WARNING: Failed to process issue #{issue_number}, will retry in next cycle"
                    )
        else:
