...
from migrations.setup_database import (
    initialize_database,
    setup_database,
)
...
def show_empty_database_guidance():
    ...
    guidance_text.append(
        "   2. This will populate your database with contacts\n", style="cyan"
    )
    guidance_text.append(
        "   3. Then you can view, search, and manage relationships\n\n", style="cyan"
    )
    guidance_text.append(
        "💡 PRT works best when you have contacts to build relationships with!",
        style="green",
    )
...
def handle_database_error(error, operation: str):
    ...
    if "no such table" in error_str:
        ...
        error_text.append(
            "   2. Or run: python -m migrations.setup_database\n", style="green"
        )
        error_text.append(
        ...
        console.print(
            Panel(error_text, title="Database Setup Required", border_style="red")
        )

    elif "database is locked" in error_str:
        console.print(
            "🔒 Database is currently locked by another process.", style="red"
        )
        console.print(
            "Please close any other PRT instances and try again.", style="yellow"
        )
        ...
        console.print(
            "\n💡 If this persists, try restarting PRT or running setup again.",
            style="yellow",
        )
    ...
    try:
        ...
        try:
            ...
            if not db.is_valid():
                return {
                    "needs_setup": True,
                    "reason": "Database is corrupted or invalid",
                }
...
def run_setup_wizard():
    ...
    try:
        ...
        console.print(
            f"Database location: {config.get('db_path', 'prt_data/prt.db')}",
            style="cyan",
        )
        console.print()
        console.print(
...
def show_main_menu(api: PRTAPI):
    ...
    table.add_row(
        "v.",
        "[bright_cyan bold]View Contacts[/bright_cyan bold] - Browse contact information",
    )
    table.add_row(
        "s.",
    ...
    table.add_row(
        "n.", "[blue bold]Manage Notes[/blue bold] - Browse and manage contact notes"
    )
    table.add_row(
        "d.",
        "[magenta bold]Manage Database[/magenta bold] - Check database stats and backup",
    )
    table.add_row(
        "i.",
    ...
    table.add_row(
        "q.", "[bright_red bold]Exit[/bright_red bold] - Exit the application"
    )
...
def handle_contacts_view(api: PRTAPI) -> None:
    ...
    try:
        ...
        if contacts:
            table = Table(
                title="Contacts", show_header=True, header_style="bold magenta"
            )
...
def handle_tag_search_results(api: PRTAPI, query: str) -> None:
        ...
        def display():
            for tag in tag_batch:
                ...
                if contacts:
                    ...
                    console.print(table)
                    console.print(
                        f"   {len(contacts)} contacts with tag '{tag_name}'",
                        style="green",
                    )
                    ...
                    console.print(
                        f"   No contacts found with tag '{tag_name}'", style="yellow"
                    )
    ...
    if result == "export":
        ...
        for info in tag_info:
            export_data.append(
                {"tag": info["tag"], "associated_contacts": info["contacts"]}
            )
...
def handle_note_search_results(api: PRTAPI, query: str) -> None:
        ...
        def display():
            for note in note_batch:
                ...
                if contacts:
                    ...
                    console.print(table)
                    console.print(
                        f"   {len(contacts)} contacts with note '{note_title}'",
                        style="green",
                    )
                    ...
                    console.print(
                        f"   No contacts found with note '{note_title}'", style="yellow"
                    )
    ...
    if result == "export":
        ...
        for info in note_info:
            export_data.append(
                {"note": info["note"], "associated_contacts": info["contacts"]}
            )
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                if index_file.exists():
                    ...
                    console.print(
                        "✅ Interactive directory generated!", style="bold green"
                    )
                    console.print(f"🌐 Open in browser: {file_url}", style="blue")
                    console.print(f"📁 Directory location: {output_dir}", style="dim")
                else:
                    console.print(
                        f"✅ Directory generated at: {output_dir}", style="green"
                    )
            else:
                console.print(
                    f"❌ Error generating directory: {result.stderr}", style="red"
                )
...
def export_search_results(
    api: PRTAPI, search_type: str, query: str, results: list
) -> None:
    ...
    console.print(
        f"💾 Exported {len(results)} {search_type} results to: {json_file}",
        style="green",
    )
...
def export_profile_images_from_results(
    results: list, export_dir: Path, timestamp: str
) -> int:
...
def handle_import_google_takeout(api: PRTAPI, config: dict) -> None:
    ...
    console.print()
    console.print(
        "This will import contacts from a Google Takeout zip file.", style="white"
    )
    ...
    if existing_files:
        console.print(
            f"🔍 Found {len(existing_files)} potential takeout file(s):", style="green"
        )
        for i, file_path in enumerate(existing_files, 1):
            console.print(f"  {i}. {file_path.name}", style="cyan")
        console.print(
            f"  {len(existing_files) + 1}. Browse for a different file", style="cyan"
        )
        console.print()
        ...
        if int(choice) <= len(existing_files):
            ...
            takeout_path = Path(
                Prompt.ask("Enter the full path to your Google Takeout zip file")
            )
    else:
        takeout_path = Path(
            Prompt.ask("Enter the full path to your Google Takeout zip file")
    ...
    try:
        ...
        if "error" in info:
            console.print(
                f"❌ Error parsing takeout file: {info['error']}", style="red"
            )
            return
        ...
        if (
            "raw_contact_count" in info
            and info["raw_contact_count"] != info["contact_count"]
        ):
            console.print(
                f"🔧 Deduplicated from {info['raw_contact_count']} raw contacts",
                style="blue",
            )
            console.print(
                f"🗑️  Removed {info['duplicates_removed']} duplicates", style="blue"
            )
        console.print(
            f"🖼️  {info['contacts_with_images']} contacts have profile images",
            style="green",
        )
        console.print()
        ...
        if success:
            console.print(
                f"✅ Successfully imported {len(contacts)} contacts!",
                style="bold green",
            )
            console.print(
                f"🖼️  {info['contacts_with_images']} contacts include profile images",
                style="green",
            )
            console.print()
        ...
        console.print(
            "Make sure the file is a valid Google Takeout zip file", style="yellow"
        )
...
def handle_import_google_contacts(api: PRTAPI, config: dict) -> None:
    ...
    try:
        ...
        if contacts:
            ...
            if success:
                console.print(
                    f"Successfully imported {len(contacts)} contacts from Google",
                    style="green",
                )
            else:
                console.print("Failed to import contacts to database", style="red")
        ...
        if db_path.exists():
            ...
            try:
                ...
                if db.is_valid():
                    ...
                    console.print(
                        f"Relationships: {db.count_relationships()}", style="green"
                    )
                else:
...
def handle_search_menu(api: PRTAPI) -> None:
    ...
    while True:
        ...
        table.add_row(
            "1.",
            "[bright_cyan bold]Search Contacts[/bright_cyan bold] - Find contacts by name",
        )
        table.add_row(
            "2.",
...
def handle_tags_menu(api: PRTAPI) -> None:
    ...
    while True:
        ...
        table.add_row(
            "3.",
            "[bright_yellow bold]Search Tags[/bright_yellow bold] - Find specific tags",
        )
        table.add_row(
            "4.",
            "[bright_red bold]Delete Tag[/bright_red bold] - Remove a tag from the system",
        )
        table.add_row(
            "b.", "[bright_magenta bold]Back to Main Menu[/bright_magenta bold]"
        )
        ...
        choice = Prompt.ask(
            "Select option", choices=["1", "2", "3", "4", "b"], default="1"
        )
...
def handle_notes_menu(api: PRTAPI) -> None:
    ...
    while True:
        ...
        table.add_row(
            "3.",
            "[bright_yellow bold]Search Notes[/bright_yellow bold] - Find specific notes",
        )
        table.add_row(
            "4.", "[blue bold]Edit Note[/blue bold] - Modify an existing note"
        )
        table.add_row(
            "5.",
            "[bright_red bold]Delete Note[/bright_red bold] - Remove a note from the system",
        )
        table.add_row(
            "b.", "[bright_magenta bold]Back to Main Menu[/bright_magenta bold]"
        )
        ...
        choice = Prompt.ask(
            "Select option", choices=["1", "2", "3", "4", "5", "b"], default="1"
        )
...
def handle_database_menu(api: PRTAPI) -> None:
    ...
    while True:
        ...
        table.add_row(
            "2.",
            "[bright_green bold]Create Backup[/bright_green bold] - Create timestamped backup",
        )
        table.add_row(
            "3.",
        ...
        table.add_row(
            "4.",
            "[blue bold]View Statistics[/blue bold] - Detailed database statistics",
        )
        table.add_row(
            "b.", "[bright_magenta bold]Back to Main Menu[/bright_magenta bold]"
        )
...
def run_interactive_cli(debug: bool = False):
    ...
    if debug:
        console.print(
            "🐛 [bold cyan]DEBUG MODE ENABLED[/bold cyan] - Using fixture data",
            style="cyan",
        )
    ...
    if not debug:
        ...
        if not health["healthy"] and health.get("needs_initialization"):
            ...
            startup_text.append(
                "Your database needs to be set up with tables.\n", style="yellow"
            )
            startup_text.append(
            ...
            startup_text.append(
                "   • Use option 3 to import Google Takeout\n", style="green"
            )
            startup_text.append(
            ...
            startup_text.append(
                "   • Then you can explore all PRT features!\n", style="green"
            )

            console.print(
                Panel(startup_text, title="Welcome to PRT", border_style="blue")
            )
            ...
            console.print(
                Panel(startup_text, title="Ready to Import", border_style="green")
            )
            console.print()
...
def main(
    ...
    debug: bool = typer.Option(
        False, "--debug", "-d", help="Run in debug mode with fixture data"
    ),
):
